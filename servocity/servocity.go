...
func getDownloadURL(ctx *spiderdata.Context, sku string, downloadurls spiderdata.DownloadEntMap) (result string) {
	...
	for key, element := range downloadurls {
		if !element.Used && strings.Contains(element.URL, skupart[0]) {
			result = element.URL
			downloadurls[key] = spiderdata.DownloadEnt{URL: ent.URL, Used: true}
			return
		}
	...
	for key, element := range downloadurls {
		if !element.Used {
			...
			if strings.Contains(key, "Instructions") &&
				...
				strings.Contains(key, ".pdf") &&
				strings.Contains(key, ".docx") &&
				strings.Contains(key, ".exe") &&
				strings.Contains(key, "arduino") &&
				strings.Contains(key, "oboclaw_") &&
				strings.Contains(key, "RoboclawClassLib") &&
				strings.Contains(key, "USBRoboclawVirtualComport") &&
				strings.Contains(key, "Pattern Information") &&
				strings.Contains(key, "bldc_hsr_") &&
				strings.Contains(key, "595644_assembly") &&
				strings.Contains(key, "Hardware Accessory Pack") &&
